<?php
	...
	if(isset($_POST['submit']))
	{
		...
		$sql = $connect->prepare("SELECT * FROM produits WHERE name_produit=?");
		$sql->execute(array($nom_produit));

		$row = $sql->rowCount();
		if($row == 0)
		{
			$sql= $connect->prepare("INSERT INTO produits(name_produit,description,nom_image) VALUES ('$nom_produit','$description','$nom_image')");
			$sql->execute();
		}else
		{
		...
		<div class="col-md-6 col-xs-12">
			<div class="panel panel-primary">
				<div class="panel panel-heading">
					<h3>Ajouter image header</h3>
				</div>
				<div class="panel panel-body">
					<div class="form-group"></div>
						<input type="hidden" name="nom_produit" value="img_header" class="form-control">
						
						<label>Phrase qui suit le logo:</label>
